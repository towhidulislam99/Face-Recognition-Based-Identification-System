...
from .models import GeobagRiverBank,FilledRiverBank,PrototypeProposed,PreparingDataset,TestingProcedures,TestingProcedures,ToonificationHeading,ToonificationImage,FaceRecognitionVideo
import os
...
def Geobag(request):
    ...
    facerecognitionvideodata = FaceRecognitionVideo.objects.all()
    ...
    data = {
        ...
        'facerecognitionvideodata': facerecognitionvideodata,
...
def facerecognitionvideopage(request):
    return render(request, 'facerecognitionvideopage.html')

def facerecognitionvideoinsert(request):
    if request.method == 'POST':
        recognition_video = request.FILES.get('recognition_video')
    
        
        facerecognitionvideo_obj = FaceRecognitionVideo()
        facerecognitionvideo_obj.recognition_video = recognition_video
        facerecognitionvideo_obj.save()
...
def facerecognitionvideodatatable(request):
    facerecognitionvideodata = FaceRecognitionVideo.objects.all()
    data = {"facerecognitionvideodata": facerecognitionvideodata}
    return render(request,'toonificationimagedatatable.html', data)

def facerecognitionvideoedit(request, id):
    facerecognitionvideodata = FaceRecognitionVideo.objects.all()
    data = {"facerecognitionvideodata": facerecognitionvideodata}
    return render(request,'facerecognitionvedioUpdate.html',data)

def facerecognitionvideoupdate(request):
    id = request.POST.get('id')

    # Get the user object by ID
    facerecognitionvideo_obj = get_object_or_404(FaceRecognitionVideo, id=id)
    
     # Check if new image and video files are provided
    if 'recognition_video' in request.FILES:
        # If a new video is provided, delete the old one if it exists
        if facerecognitionvideo_obj.video:
            os.remove(facerecognitionvideo_obj.recognition_video.path)
        facerecognitionvideo_obj.recognition_video = request.FILES['recognition_video']
   
    facerecognitionvideo_obj.save()
    return redirect('Geobagdatatable')

def deletefacerecognitionvideo(request, id):
    facerecognitionvideo_obj = get_object_or_404(FaceRecognitionVideo, id=id)
    facerecognitionvideo_obj.delete()
    return redirect('Geobagdatatable')
